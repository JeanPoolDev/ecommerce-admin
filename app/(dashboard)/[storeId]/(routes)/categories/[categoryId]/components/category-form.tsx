...
export const CategoryForm: React.FC<CategoryFormProps> = ({
  ...
  const description = initialData ? "Editar un Categoria" : "Agregar una nueva Categoria";
  const toastMessage = initialData ? "Actualizar Categoria" : "Crear Categoria";
  ...
  const onSumit = async (data: CategoryFormValues) => {
    try{
      setLoading(true);
      if(initialData){
  ...
  const onDelete = async () => {
    try{
      ...
      toast.error("Asegúrese de eliminar todos los Categorias primero")
    } finally {
      setLoading(false);
      setOpen(false);
    }
